...
class pick_and_place
{
  public:
    ...
    void subpubclient()
    {
      object_position = node_handle.serviceClient<arm_test::go>("/go", false);
      object_position.waitForExistence();

      robot_motion = node_handle.advertise<std_msgs::Bool>("/arm1/robot_state", 1, true);
      planning_scene_diff_client = node_handle.serviceClient<moveit_msgs::ApplyPlanningScene>("/arm1/apply_planning_scene");
...
// and send the diffs to the planning scene via a service call:
    ...
    target_pose2.position.x = 0.72;
    target_pose2.position.y = 0;
    target_pose2.position.z = 0.73;
  ...
  void start_state(moveit::planning_interface::MoveGroup &group)
  {
    ...
    ROS_INFO("Visualizing robot coming back to its original state %s",success?"":"FAILED");
    std::cout<<group.move().SUCCESS<<std::endl;

    std_msgs::Bool state;
    if (group.move().SUCCESS)
    {
      state.data = true;
      robot_motion.publish(state);
    }
...
//request of grasping object position and calling the service
  ...
  arm_test::go positionsrv;
  positionsrv.request.arm_type = "arm1";
  
  while(positionsrv.response.go_vector.empty())
    {
      sleep(0.5);
      pp.object_position.call(positionsrv);
    }

  std::vector<arm_test::graspposition> target_vec;
  int i=0;

  while (pp.object_position.call(positionsrv)) //another condition that terminates the process when the go_vector is at the end or waits until the next index is filled
  {
    target_vec = positionsrv.response.go_vector;
...
//returning to start state
    ...
    pp.object_position.call(positionsrv);
    while ((positionsrv.response.go_vector[positionsrv.response.go_vector.size()-1].object_names==target_vec[i].object_names)) //second condition for #2 optimization
    {
      sleep(0.5);
      pp.object_position.call(positionsrv);
    }
